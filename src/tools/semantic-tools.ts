...
export function getOperationDescription(operation: string): string {
  const descriptions: Record<string, string> = {
    ...
    system: 'ℹ️ System operations - info: server details, commands: list available command IDs, fetch_web: retrieve and process web content, execute: run a command-palette command by exact ID (opt-in and allowlisted — disabled by default; see ADR-204). execute is fire-and-forget: success means the command was DISPATCHED, not that it finished. Some commands open a dialog that needs a person; when detected, the result sets awaitingUserInteraction=true — treat that as "a human must act in Obsidian," not as completion.',
